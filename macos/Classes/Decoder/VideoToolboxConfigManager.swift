...
/// VideoToolbox 配置管理器 (macOS)
///
/// 负责配置 FFmpeg 的 VideoToolbox 硬件解码选项，
...
public class VideoToolboxConfigManager {
    ...
    public struct SupportedCodecs {
        ...
        public init(h264: Bool = true, hevc: Bool = false, vp9: Bool = false, av1: Bool = false, prores: Bool = false) {
            self.h264 = h264
            self.hevc = hevc
            self.vp9 = vp9
            self.av1 = av1
    ...
    public struct DecoderConfiguration {
        ...
        public init(
            ...
            self.enableProRes = enableProRes
    ...
    public func getAvailableHardwareDecoders() -> [HardwareDecoderCapability] {
        ...
        decoders.append(HardwareDecoderCapability(
            ...
            codecName: "ProRes 422",
            isSupported: true,  // macOS 原生支持
            maxWidth: 8192,
            maxHeight: 4320,
            supportedProfiles: ["Proxy", "LT", "422", "HQ"]
        ))
    ...
    public func getSupportedCodecs() -> SupportedCodecs {
        return SupportedCodecs(
            ...
            vp9: false,  // macOS 不支持 VP9 硬解
            av1: false,  // macOS 暂不支持 AV1 硬解
            prores: true // macOS 原生支持 ProRes
        )
    }
    ...
    /// 获取 FFmpeg 的解码配置选项
    public func getFFmpegOptions(for config: DecoderConfiguration) -> [String: Any] {
        var options: [String: Any] = [:]
        
        // VideoToolbox 硬解开关
        if config.enableHardwareDecode {
