...
2. **加载设计文档**: 从 FEATURE_DIR 读取: 
   ...
   - **可选**: feasibility.md(技术方案评估、POC清单、风险缓解), data-model.md(实体), contracts/(API 端点), research.md(决策), quickstart.md(测试场景)
...
4. **来自 feasibility.md (如果存在)**: 
   - POC 验证清单 → 基础阶段的 POC 验证任务(在用户故事之前)
   - 风险缓解措施 → 相关故事阶段的应对任务
   - 技术方案决策 → 影响任务分解和实现顺序
...
6. **排序**: 
   - 阶段 1: 设置(项目初始化)
   - 阶段 2: 基础(阻塞先决条件 - 必须在用户故事之前完成，包括 POC 验证任务)
