...
public class ProviderRegistryImpl {
    ...
    // 默认 Provider
    private var _dataSourceProvider: IOSDataSourceProvider?
    private var _decoderProvider: IOSDecoderProvider?
    private var _rendererProvider: IOSRendererProvider?
    ...
    public func configure(with registrar: FlutterPluginRegistrar) {
        ...
        _dataSourceProvider = IOSDataSourceProvider(registrar: registrar)
    }
    ...
    private func registerDefaultProviders() {
        // DataSource Provider
        let dataSourceProvider = IOSDataSourceProvider()
        _dataSourceProvider = dataSourceProvider
        registerProvider(
            dataSourceProvider,
            info: ProviderInfo(
                type: .dataSource,
                name: "IOSDataSourceProvider",
                version: "1.0.0",
                priority: 0
            )
        )
        ...
        // Decoder Provider
        let decoderProvider = IOSDecoderProvider()
        _decoderProvider = decoderProvider
        registerProvider(
            decoderProvider,
            info: ProviderInfo(
                type: .decoder,
                name: "IOSDecoderProvider",
                version: "1.0.0",
                priority: 0
            )
        )
        ...
        // Renderer Provider
        let rendererProvider = IOSRendererProvider()
        _rendererProvider = rendererProvider
        registerProvider(
            rendererProvider,
            info: ProviderInfo(
                type: .renderer,
                name: "IOSRendererProvider",
                version: "1.0.0",
                priority: 0
            )
        )
    ...
    /// 获取数据源 Provider
    public func getDataSourceProvider() -> IOSDataSourceProvider {
        return getProvider(.dataSource) ?? _dataSourceProvider ?? IOSDataSourceProvider()
    }
    
    /// 获取解码器 Provider
    public func getDecoderProvider() -> IOSDecoderProvider {
        return getProvider(.decoder) ?? _decoderProvider ?? IOSDecoderProvider()
    }
    
    /// 获取渲染器 Provider
    public func getRendererProvider() -> IOSRendererProvider {
        return getProvider(.renderer) ?? _rendererProvider ?? IOSRendererProvider()
    }
